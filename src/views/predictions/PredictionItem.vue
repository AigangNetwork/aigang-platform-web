  ...
  <router-link :to="{ name: 'PredictionInfo', params: { id: item.id}}">
    <div class="aig-data">
        <div class="aig-data-head">
          <div class="status" v-if="item.status === 'published'">{{ $t('predictions.predictionsList.prediction.votingTill') }}: <Date :dateUtc="item.forecastEndUtc" /></div>
          <div class="status" v-if="item.status === 'paused'">{{ $t('predictions.predictionsList.prediction.paused') }}</div>
          <div class="status" v-if="item.status === 'canceled'">{{ $t('predictions.predictionsList.prediction.canceled') }}</div>
          <div class="status" v-if="item.status === 'resolved'">{{ $t('predictions.predictionsList.prediction.ended') }}</div>
