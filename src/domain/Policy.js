...
export default class Policy {
  async initializeItem (details) {
    ...
    this.startUtc = moment.unix(details.utcStart)
    this.endUtc = moment.unix(details.utcEnd)
  ...
  async initialize (details) {
    ...
    this.payoutUtc = moment.unix(details[2]).format(details.utcPayoutDate)
    this.properties = details.properties
    this.claimProperties = details.claimProperties
    this.address = details.address
    this.policyTermInDays = Math.floor(
      moment.duration(moment.unix(details.utcEnd).diff(moment.unix(details.utcStart))).asDays()
    )
    this.walletAddress = details.walletAddress
    this.createUtc = moment.unix(details[2]).format(details.created)
  }
